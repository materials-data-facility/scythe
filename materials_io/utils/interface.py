...
def execute_parser(name, group, global_context=None,
                   adapter_context: Union[None, dict] = None,
                   parser_context: Union[None, dict] = None,
                   adapter: Union[None, str] = None,
                   adapter_map: Union[None, str] = None,
                   default_adapter: Union[None, str] = None):
    """Invoke a parser on a certain group of data AND apply appropriate adapter to metadata.
    ...
    Args:
        ...
        adapter_map (dict): keys are adapter/parser names, values are adapters
    ...
    metadata = get_parser(name).parse(group, my_parser_context)
    if adapter is not None:
        parser = get_parser(name)
        adapter_map = get_adapter_map(adapter_map=adapter_map, parsers=[parser])
        ...
        if adapter_name is not None:
            ...
            # Note, here we don't care about the metadata=None case. It will just return as such.
    return metadata
